...
import { GetGoodsInfo, uploadImg, deleteImg, updateGoods } from '@/api/goods'
import { getClassifyList } from '@/api/classify'
export default {
  name: 'edit',
  created () {
    this.id = this.$route.params.id
    this.handelGetGoodsInfo(this.id)
  ...
  methods: {
    ...
    async handelGetGoodsInfo (id) {
      let data = await GetGoodsInfo({ id })
      if (data && data.id) {
        this.goods = {
          ...data,
          cid: data.classify.cid
        }
      }
    },
    handleAddParams () {
